...
After editing or creating a new diagram, you can test your changes by opening them in a browser. Before you do this, you need to run `npm install from the `api/src/docs/js` directory, this will download the dependencies, in particular, `svg.js`. Having done that, you can then open [`api/src/docs/js/index.html`](api/src/docs/js/index.html), this will show you all the rendered diagrams. No generation step is required to view these diagrams, you can simply hit refresh in the browser after making changes.

=== Generating diagrams

Once you have edited the diagrams file, before committing, you will need to generate new PNG files for the diagrams. This can be done by running `node [api/src/docs/js/generate-marble-diagrams.js](api/src/docs/js/generate-marble-diagrams.js)`. This will output all the diagrams to `api/src/main/java/org/eclipse/microprofile/reactive/streams/doc-files`, the directory that javadoc supports for placing images. You can then run `mvn package` to generate the javadocs and see the results.
