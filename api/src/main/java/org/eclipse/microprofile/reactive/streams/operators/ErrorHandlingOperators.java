...
public interface ErrorHandlingOperators<T> {
     ...
     * instead of invoking its subscriber's <code>onError</code> method, it will instead emit the return value of the
     * passed function. This operator prevents errors from propagating or to supply fallback data should errors be
     * encountered.
     *
     * @param errorHandler the function returning the value that needs to be emitting instead of the error.
     ...
     * @return The new stream builder.
     */
    ErrorHandlingOperators<T> onErrorResumeWithRsPublisher(Function<Throwable, ? extends Publisher<? extends T>> errorHandler);
}
