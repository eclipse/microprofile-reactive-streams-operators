...
Finally, sometimes there is a need to signify completion, either succesfully or with an error, with no value. For example, a database update may not return any value. In such case, `CompletionStage<Void>` should be used, which gets redeemed with `null` when successful.
